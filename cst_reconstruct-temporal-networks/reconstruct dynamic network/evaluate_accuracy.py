...
def CR(reconstructed_adjacency):
    ...
    Rtriu = np.triu(reconstructed_adjacency).T #上三角-转置->下三角
    Rtril = np.tril(reconstructed_adjacency) #下三角
    conflict = CR_tri(Rtriu, Rtril)
    
    return conflict

def CR_tri(upper_adjacency, lower_adjacency):
    """
    ...
    """
    # 确保输入的邻接矩阵是二维的numpy数组
    upper_adjacency = np.array(upper_adjacency)
    lower_adjacency = np.array(lower_adjacency)

    # 确保邻接矩阵的形状一致
    if upper_adjacency.shape != lower_adjacency.shape:
        raise ValueError("SRAC:原始网络和重构网络的邻接矩阵形状不一致")
    correct_predictions = np.sum(upper_adjacency != lower_adjacency)
    total_edges = upper_adjacency.size/2
    accuracy = correct_predictions / total_edges
    return accuracy